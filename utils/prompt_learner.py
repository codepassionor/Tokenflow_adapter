...
class PrefixToken(nn.Module):
    def __init__(self, share_token_num):
        ...
        self.projection = nn.Linear(768, 768) #(trainable)
        self.share_token = nn.Parameter(torch.randn(share_token_num, 768)) # share_token -> unshared token
        # unshared token , sub decrease, average pooling(trainbale) , concat shared token(trainable)
    ...
    def forward(self, image, clip_model, clip_processor, sub=2):
