...
def main(config_source_experiment_name, config_prompts, config_negative_prompt, image_name):
    parser = argparse.ArgumentParser()
    parser.add_argument('--config', default = 'configs/pnp/pnp.yaml')
    ...
    parser.add_argument("--save_all_features", default=False, action="store_true", help="if set to true, saves all feature maps, otherwise only saves those necessary for PnP")
    opt = parser.parse_args()
    exp_config = OmegaConf.load(opt.config)
    exp_config.source_experiment_name = config_source_experiment_name
    exp_config.prompts = config_prompts
    exp_config.negative_prompt = config_negative_prompt
    ...
    translation_folder = exp_config.prompts.replace(' ', '_')
    outpath = os.path.join(f"{exp_path_root}/{exp_config.source_experiment_name}/translations", f"{exp_config.scale}_{translation_folder}")
    ...
    predicted_samples_path = os.path.join(outpath, f"predicted_samples_{out_label}")
    feature_maps_path = os.path.join(outpath, "feature_maps", image_name)
    ...
    def load_target_features():
        ...
        source_experiment_out_layers_path = os.path.join(exp_path_root, exp_config.source_experiment_name, "feature_maps", image_name)
        source_experiment_qkv_path = os.path.join(exp_path_root, exp_config.source_experiment_name, "feature_maps", image_name)
        ...
        with precision_scope("cuda"):
            with model.ema_scope():
                ...
                for k, x_sample in enumerate(x_image_torch):
                    x_sample = 255. * rearrange(x_sample.cpu().numpy(), 'c h w -> h w c')
                    img = Image.fromarray(x_sample.astype(np.uint8))
                    img.save(os.path.join(outpath, f"{out_label}_sample_{sample_idx}.png"))
                    sample_idx += 1
                '''
