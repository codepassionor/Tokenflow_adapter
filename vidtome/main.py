...
if __name__ == "__main__":
    ...
    for i in range(13):
        ...
        for prompt in config['generation']['prompt']:
            ...
            print("Start inversion!")
            inversion = Inverter(pipe, scheduler, config)
            inversion(config.input_path, config.inversion.save_path, lora_begin, lora_end)
            ...
            frame_ids = get_frame_ids(
                config.generation.frame_range, config.generation.frame_ids)
            generator(config.input_path, config.generation.latents_path,
                    f'{lora_begin}/' + config.generation.output_path + '/' + prompt_id + "/base", frame_ids=frame_ids, lora_begin=lora_begin, lora_end=lora_end)
            ...
            frame_ids = get_frame_ids(
                config.generation.frame_range, config.generation.frame_ids)
            generator(config.input_path, config.generation.latents_path,
                    f'{lora_begin}/' + config.generation.output_path + '/' + prompt_id + "/lora", frame_ids=frame_ids, lora_begin=lora_begin, lora_end=lora_end)
