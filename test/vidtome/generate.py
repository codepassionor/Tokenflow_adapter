...
import os
from transformers import logging, CLIPModel, CLIPProcessor
...
class Generator(nn.Module):
    def __init__(self, pipe, scheduler, config, givenprompt):
        super().__init__()
    ...
    @torch.no_grad()
    def get_text_embeds_input(self, prompt, negative_prompt, use_prefixtoken):
        ...
        if use_prefixtoken:
            prefix_token = self.prompt_learner(self.frames[0], self.clip_model, self.clip_processor)
            prefix_token = prefix_token.repeat(3, 1, 1).half()
            text_embeds = torch.cat([prefix_token, text_embeds], dim=1)
        return text_embeds
    ...
    @torch.no_grad()
    def ddim_sample(self, x, conds, lora_begin, lora_end):
    ...
    @torch.no_grad()
    def __call__(self, data_path, latent_path, output_path, frame_ids, lora_begin, lora_end, use_prefixtoken):
        ...
        for edit_name, edit_prompt in self.prompt.items():
            ...
            conds = self.get_text_embeds_input(edit_prompt, self.negative_prompt, use_prefixtoken)
            # Comment this if you have enough GPU memory
            clean_latent = self.ddim_sample(self.init_noise, conds, lora_begin, lora_end)
            torch.cuda.empty_cache()
