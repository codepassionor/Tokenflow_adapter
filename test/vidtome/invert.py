...
class Inverter(nn.Module):
    ...
    @torch.no_grad()
    def ddim_inversion(self, x, conds, save_path, lora_begin, lora_end):
    ...
    @torch.no_grad()
    def __call__(self, data_path, save_path, lora_begin, lora_end, use_prefixtoken):
        ...
        if use_prefixtoken:
            image = frames[0]
            prefix_token = self.prompt_learner(image, self.clip_model, self.clip_processor)
            prefix_token = prefix_token.repeat(len(frames), 1, 1).half()
            conds = torch.cat([prefix_token, conds], dim=1)
        ...
        inverted_x = self.ddim_inversion(latents, conds, save_path, lora_begin, lora_end)
        save_config(self.config, save_path, inv = True)
        if self.recon:
            latent_reconstruction = self.ddim_sample(inverted_x, conds, lora_begin, lora_end)
