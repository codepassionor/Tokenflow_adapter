...
from transformers import logging, CLIPProcessor, CLIPModel
...
class TokenFlow(nn.Module):
    ...
    @torch.no_grad()
    def get_text_embeds(self, prompt, negative_prompt, use_prefixtoken, batch_size=1):
        ...
        if use_prefixtoken:
            text_embeddings = torch.concat([self.prefix_token.repeat(2, 1, 1), text_embeddings], dim=1)
    ...
    def batched_denoise_step(self, x, t, indices):
        ...
        denoised_latents = []
        pivotal_idx = torch.ones((len(x)//batch_size,), dtype=torch.int64) + torch.arange(0,len(x),batch_size) # torch.randint(batch_size, (len(x)//batch_size,))
    ...
    def save_vae_recon(self):
        os.makedirs(f'{self.config["output_path"]}/vae_recon_lora', exist_ok=True)
        decoded = self.decode_latents(self.latents)
        for i in range(len(decoded)):
            T.ToPILImage()(decoded[i]).save(f'{self.config["output_path"]}/vae_recon_lora/%05d.png' % i)
    ...
    def edit_video(self, lora_begin, lora_end):
        os.makedirs(f'{self.config["output_path"]}/img_ode_lora', exist_ok=True)
        self.save_vae_recon()
        ...
        edited_frames = self.sample_loop(noisy_latents, torch.arange(self.config["n_frames"]), lora_begin, lora_end)
    ...
    def sample_loop(self, x, indices, lora_begin, lora_end):
        os.makedirs(f'{self.config["output_path"]}/img_ode_lora', exist_ok=True)
        for i, t in enumerate(tqdm(self.scheduler.timesteps, desc="Sampling")):
                #print(self.scheduler.timesteps)
                if lora_begin <= t < lora_end:
                    if not self.lora_inuse:
                        self.lora_inuse = True
                        self.pipe.enable_lora()
                else:
                    if self.lora_inuse:
                        self.lora_inuse = False
                        self.pipe.disable_lora()
                x = self.batched_denoise_step(x, t, indices)
        
        decoded_latents = self.decode_latents(x)
        for i in range(len(decoded_latents)):
            T.ToPILImage()(decoded_latents[i]).save(f'{self.config["output_path"]}/img_ode_lora/%05d.png' % i)

        return decoded_latents


def run(config, lora_begin, lora_end):
    #seed_everything(config["seed"])
    print(config)
    editor = TokenFlow(config)
    editor.edit_video(lora_begin, lora_end)
