...
@torch.no_grad()
def ddim_loop(pipeline, ddim_scheduler, latent, num_inv_steps, prompt, lora_begin, lora_end, normal_infer=False):
    context = init_prompt(prompt, pipeline)
    uncond_embeddings, cond_embeddings = context.chunk(2)
    all_latent = [latent]
    latent = latent.clone().detach()
    for i in tqdm(range(num_inv_steps)):
        if pipeline.withlora:
            if lora_begin <= t < lora_end:
                if not pipeline.lora_inuse:
                    pipeline.lora_inuse = True
                    pipeline.enable_lora()
            else:
                if pipeline.lora_inuse:
                    pipeline.lora_inuse = False
...
@torch.no_grad()
def ddim_inversion(pipeline, ddim_scheduler, video_latent, num_inv_steps, prompt="", lora_begin=0, lora_end=300, normal_infer=False):
    ddim_latents = ddim_loop(pipeline, ddim_scheduler, video_latent, num_inv_steps, prompt, lora_begin, lora_end, normal_infer=normal_infer)
    return ddim_latents
